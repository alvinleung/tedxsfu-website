...
const Logo = ({ isEnterAnimationDone }: Props) => {
  ...
  const conditionNeue = (
    width >= breakpoints["2xl"] && {minWidth: "50vw", width: "min(83.75dvh)", maxWidth:"calc((4 * (100vw - 9rem) / 8) + 3rem)"}
    ||
    width >= breakpoints.lg && {minWidth: "64vw", width: "min(83.75dvh)", maxWidth:"calc((4 * (100vw - 7rem) / 6) + 3rem)"}
    ||
    width >= breakpoints.md && {minWidth: "", width: "min(83.75dvh)", maxWidth:"calc((4 * (100vw - 7rem) / 6) + 3rem)"}
    ||
    {minWidth: "18rem", width: "min(83.75dvh)", maxWidth: "calc(100vw - 2rem)"}
  )
  ...
  return (
    ...
    >
      <svg 
        className="flex flex-col gap-4 max-sm:mt-16 fill-white"
        style={{
          ...conditionNeue,
